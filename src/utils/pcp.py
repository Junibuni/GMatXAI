# https://github.com/gregornickel/pcp/blob/main/plot.py#L131
...
def load_dataframe(df: pd.DataFrame, first_col: str='tag', last_col: str='val_mae'):
    if first_col not in df.columns or last_col not in df.columns:
        raise ValueError("Specified columns not found in DataFrame.")

    middle_cols = [col for col in df.columns if col not in {first_col, last_col}]
    ordered_df = df[[first_col] + middle_cols + [last_col]].fillna(0.0)
    
    header = ordered_df.columns.tolist()
    data = ordered_df.values.tolist()
